...
$(window).on('load', function() {
    /** calculates riesgo of comuna */	
    $.ajax({
		url: window.location.origin + '/admin/reportes/comunales/riesgo',
		type: 'GET',
		success: function(response) {
			...
			let total_combustible = response['combustible_total']['total'];
			...
			let totalInterfaces = response['interfaces'].length;
			let meanLimpiezaTechos = 0;
			let meanResiduosAgricolas = 0;
			let meanResiduosForestales = 0;
			let meanResiduosDomesticos = 0;
			let meanResiduosIndustriales = 0;
		    ...
		    /*for (let key in response['pendiente']) {
				if (response['pendiente'][key]['gridcode'] == 1) pen_percentage_clase_1 += ((parseInt(response['pendiente'][key]['gridcode']) * parseFloat(response['pendiente'][key]['pc_superficie'])) / 100);
				else if (response['pendiente'][key]['gridcode'] == 2) pen_percentage_clase_2 += ((parseInt(response['pendiente'][key]['gridcode']) * parseFloat(response['pendiente'][key]['pc_superficie'])) / 100);
				else if (response['pendiente'][key]['gridcode'] == 3) pen_percentage_clase_3 += ((parseInt(response['pendiente'][key]['gridcode']) * parseFloat(response['pendiente'][key]['pc_superficie'])) / 100);
			...
			/** calculates vegetación combustible */
			for (let key in response['combustible']) {
                res_combustible = res_combustible + (parseInt(response['combustible'][key]['puntaje']) * (parseFloat(response['combustible'][key]['hectareas'])/total_combustible));
			}
			...
			response['interfaces'].forEach(element => {
				meanLimpiezaTechos = meanLimpiezaTechos + parseInt(element.limpieza_techo);
				meanResiduosAgricolas = meanResiduosAgricolas + parseInt(element.residuos_agricolas);
				meanResiduosForestales = meanResiduosForestales + parseInt(element.residuos_forestales);
				meanResiduosDomesticos = meanResiduosDomesticos + parseInt(element.residuos_domesticos);
				meanResiduosIndustriales = meanResiduosIndustriales + parseInt(element.residuos_industriales);
			});
			...
			if ((res_prevalence != null) && (res_pen_average != null) && (res_combustible != null) && (res_pbl_density != null) && (!isNaN(res_techo))) {
				...
				vulnerabilidad = Math.round(0.2*res_pbl_density + 0.1*res_techo + 0.4*res_otros_comb + 0.3*meanLimpiezaTechos);
                ...
                $('#val-amenaza-comuna').html(amenaza);
                $('#val-vulnerabilidad-comuna').html(vulnerabilidad);

				if (riesgo <= 25) {
					$('#val-riesgo-comuna').trigger('configure', {
						'fgColor': colors[0],
						'min': 0,
						'max': 100
					}).val(riesgo).trigger('change');
					$('#val-riesgo-comuna').css({ 'color': colors[0] });
					...
					$('#val-riesgo-comuna').css({ 'color': colors[1] });
					...
					$('#val-riesgo-comuna').css({ 'color': colors[2] });
				} else if (riesgo > 75) {
					$('#val-riesgo-comuna').trigger('configure', {
						'fgColor': colors[3],
						'min': 0,
					...
					$('#val-riesgo-comuna').css({ 'color': colors[3] });
				}
			}
		},
    ...
    $.ajax({
		url: window.location.origin + '/admin/reportes/comunales/riesgo/prevalencia',
		type: 'GET',
		success: function(response) {
...
function getDetailsGrupoCausas(btn) {
    ...
    $.ajax({
        url: window.location.origin + '/admin/reportes/comunales/causas/grupos', 
        type: 'POST',             
        data: {
