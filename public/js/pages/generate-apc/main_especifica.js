...
function updateMeasure(id) {
    ...
    let zonas = $('#objetive-zones'+idButtonSave).val();
    let responsable = $('#responsible-entity'+idButtonSave).val();
	let contacto = $('#responsible-name'+idButtonSave).val();
    let fechaInicio = $('#start-date'+idButtonSave).val();
	let fechaTermino = $('#end-date'+idButtonSave).val();
	let avance = $('#progress-level'+idButtonSave).val();
    if ((grupo != '') && (zonas != '') && (responsable != '') && (contacto != '') && (fechaInicio != '') && (fechaTermino != '') && ((avance >= 0) && (avance <= 100))) {
        ...
        $.ajax({
            ...
            data: {
                ...
                'medida-zonas-objetivo': zonas,
                'medida-responsable': responsable,
                'medida-contacto-responsable': contacto,
                'medida-fecha-inicio': fechaInicio,
                'medida-fecha-termino': fechaTermino,
        ...
        success: function(response) {
            ...
            if (response == 1) {
                swal({
                    ...
                    window.location.href = window.location.origin + '/admin/apc/especifica/listar';
                });
            }
            else {
                swal({
        ...
        success: function(response) {
            ...
            if (response == 1) {
                swal({
                    ...
                    window.location.href = window.location.origin + '/admin/apc/especifica/editar/' + APCid;
                });
            }
            else {
                swal({
...
/*function exportarPDF() {
    let divMedidasEspecificas = document.getElementById('table-medidas-especificas');
    $("#table-body-medidas-especificas tr").each(function(){
